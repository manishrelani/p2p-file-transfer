...
import { FileMetaData, ICECandidate, ResponseInterface, Room, SessionDescriptionData } from './interface.js';
...
io.on('connection', (socket) => {
    ...
    socket.on('create-room', (listOfFiles: FileMetaData[], callback) => {
    ...
    socket.on('join-room', (roomCode, callback) => {
        ...
        if (!room) {
            ...
            callback({ isSucess: false, data: 'Room not found' } as ResponseInterface);
            return;
        ...
        if (room.receiver) {
            ...
            callback({ isSucess: false, data: 'Room is already full' } as ResponseInterface);
            return;
    ...
    socket.on('offer-created', (data: SessionDescriptionData) => {
