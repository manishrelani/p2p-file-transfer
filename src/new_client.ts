...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sendBtn) {
        sendBtn.addEventListener('click', function () {
            ...
            setMode('send', this as HTMLButtonElement);
        });
    } else {
    ...
    if (receiveBtn) {
        receiveBtn.addEventListener('click', function () {
            ...
            setMode('receive', this as HTMLButtonElement);
        });
    } else {
    ...
    let currentMode: 'send' | 'receive' = 'send';


    let isInitiator: boolean = false;
    let room: intr.Room | null = null;

    let currentFileList: intr.FileData[] = [];
    let localPeerConnection: RTCPeerConnection | null = null;
    let dataChannel: RTCDataChannel | null = null;

    let writableStreams: Record<string, intr.FileSystemWritableFileStream> = {};
    let fileTransferStatus: Record<string, intr.FileTransferStatus> = {};
    let fileReceiverStatus: Record<string, intr.FileReceiveStatus> = {};


    let allFilesTransferState: intr.TransferState = {
        totalFileSent: 0,
        isInProgress: false,
    ...
    function initializeSocket() {
        ...
        socket.on('peer-joined', (code: string) => {
    ...
    function processFiles(files: File[]) {
        ...
        socket?.emit('create-room', currentFileList.map(file => file.toMetaData()), (roomData: intr.Room) => {
    ...
    function joinRoom() {
        ...
        socket.emit('join-room', code, (response: intr.ResponseInterface) => {
    ...
    function displaySelectedFiles(files: intr.FileMetaData[]) {
    ...
    async function setupPeerConnection(roomCode: string) {
        ...
        peerConnection.onicecandidate = (event: RTCPeerConnectionIceEvent) => {
            if (event.candidate) {
                ...
                const candidate: intr.ICECandidate = {
                    roomCode: roomCode,
            ...
            if (event.data === '200') {
                ...
                try {
                    ...
                    if (data.type === 'file-chunk') {
                        writeToFile(data.data as intr.FileChunks);
                    } else {
    ...
    async function writeToFile(chunkData: intr.FileChunks) {
        ...
        try {
            ...
            const fileStatus = fileReceiverStatus[chunkData.fileId] ??= {
                progress: 0,
                isComplete: false,
    ...
    async function createOffer(peerConnection: RTCPeerConnection) {
        if (room) {
            ...
            const offerData: intr.SessionDescriptionData = {
                roomCode: room.code,
    ...
    async function createAnswer(peerConnection: RTCPeerConnection, offer: RTCSessionDescriptionInit) {
        ...
        try {
            ...
            const answerData: intr.SessionDescriptionData = {
                roomCode: room!.code,
        ...
        if (room) {
            if ('showDirectoryPicker' in window) {
                try {
                    ...
                    alert('Error accessing the directory: ' + (err as Error).message);
                }
    ...
    async function sendFile(fileData: intr.FileData) {
        ...
        try {
            ...
            const fileStatus = fileTransferStatus[fileData.id] ??= {
                progress: 0,
                isComplete: false,
            ...
            while (fileStatus.offset < fileData.size) {
                ...
                const data: intr.FileChunks = { "fileId": fileData.id, "chunk": arrayBuffer, "offset": fileStatus.offset };
